...
# get the parent directory (assuming the modules are in ../modulus/sym/)
current_dir = os.path.dirname(os.path.abspath(__file__))
parent_dir = os.path.abspath(os.path.join(current_dir, '../../modulus/sym'))
...
#     add_hydra_run_path,
# )
from modulus.sym.hydra.utils import instantiate_optim, instantiate_sched, instantiate_agg, add_hydra_run_path
...
class Trainer(AdamMixin, AdaHessianMixin, BFGSMixin):
    ...
    def _train_loop(
            self,
            domain_index,  # to check if the time window corresponds to the IC or later steps
        ...
        # using larger max_steps for the IC to converge well
        if domain_index == 0:
            max_steps = self.max_steps * 4
        else:
        ...
        with ExitStack() as stack:
            ...
            for step in range(self.initial_step, max_steps + 1):
